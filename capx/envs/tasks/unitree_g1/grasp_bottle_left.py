...
PROMPT = """
You are controlling the left arm of a real Unitree G1 robot as a single-arm robot with the API described below. The configured PyRoKi target is left_hand_palm_link.
Goal: grasp a plastic water bottle and lift it off the table.

Key rules:
- Use the left Dex3 hand as the gripper. Available hand APIs: set_gripper_trigger_squeeze(trigger, squeeze), open_gripper(), close_gripper(), close_index_pinch(), close_middle_pinch(), and move_hand_joints(joints).
...
- After the grasp/lift completes, call move_to_pregrasp_side_pose() so the final arm target is the configured left-side lifted pose.
...
# Park the grasping arm at the configured left-side lifted pregrasp pose before
# the launch exits, so Gateway timeout/damping starts from this safe side target.
move_to_pregrasp_side_pose()
"""


class UnitreeG1LeftGraspBottleCodeEnv(CodeExecutionEnvBase):
    """High-level code environment for grasping a plastic bottle with Unitree G1 left arm."""
...
__all__ = [
    "UnitreeG1LeftGraspBottleCodeEnv",
]
